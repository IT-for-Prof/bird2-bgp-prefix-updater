...
# Generated BIRD include holding `define OWN_INFRA = [...]`, derived from
...
def write_own_infra_conf(
    own: List[ipaddress.IPv4Network], path: Optional[str] = None
) -> None:
    """Write the BIRD `define OWN_INFRA = [...]` include from the inventory.

    Keeps the L2 export-filter list in sync with OWN_INFRA_FILE automatically,
    so the git-tracked bird.conf holds no deployment-local networks. BIRD
    rejects an empty list, but load_own_infra() already fails closed on empty,
    so `own` is guaranteed non-empty here.
    """
    if path is None:
        path = OWN_INFRA_CONF
    nets = ", ".join(f"{n.with_prefixlen}+" for n in own)
    content = (
        "# Generated by prefix_updater.py from the own-infra inventory.\n"
        "# Do NOT edit by hand; edit /etc/bird/own-infra.lst instead.\n"
        f"define OWN_INFRA = [ {nets} ];\n"
    )
    atomic_write(path, content)
    print(f"Wrote OWN_INFRA include ({len(own)} prefixes) to {path}")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt == MAX_RETRIES:
                print(
                    f"Error: Failed to download {source['name']} after {MAX_RETRIES} attempts: {e}"
                )
                ...
                if os.path.exists(cache_path):
                    ...
                    if cache_age <= STALE_CACHE_MAX_AGE:
                        ...
                        if stale_data:
                            print(
                                f"WARNING: Using stale cache for {source['name']} (age: {cache_age / 3600:.1f}h)"
                            )
                            return stale_data
                    else:
                        print(
                            f"WARNING: Stale cache for {source['name']} too old ({cache_age / 86400:.1f} days), discarding"
                        )
                return None
            print(
                f"Attempt {attempt} failed for {source['name']}: {e}. Retrying in {RETRY_DELAY * (2 ** (attempt - 1))}s..."
            )
...
def smoke_test_bird(temp_bird_file: str) -> bool:
    ...
    try:
        ...
        if old_include_pattern not in conf_data:
            print(
                f"Warning: Could not find '{old_include_pattern}' in {BIRD_CONF}. Smoke test might be inaccurate."
            )
...
Examples:
  ...
  %(prog)s --force-refresh        # Ignore cache and download all sources fresh
    ...
    parser.add_argument(
        "--check",
        type=str,
        help="Check which source contains a specific IP or CIDR (diagnostic mode)",
    )
    parser.add_argument(
        "--force-refresh",
        action="store_true",
    ...
    for src in SOURCES:
        ...
        if not any_success or (failed_urls and src.get("require_all_urls")):
            ...
            old_count = sum(
                1 for comms in old_routes.values() if src["community_suffix"] in comms
            )
            source_stats.append(
                (src["name"], src["community_suffix"], old_count, "FALLBACK")
            )
        ...
        source_stats.append(
            (src["name"], src["community_suffix"], len(collapsed), "OK")
        )
    ...
    # Regenerate the BIRD OWN_INFRA include from the same inventory (single
    # source of truth for L1 subtraction and L2 export filters). The '+' suffix
    # matches each prefix and all more-specifics.
    write_own_infra_conf(own_infra)
    ...
    sorted_cidrs = sorted(
        all_routes.keys(),
        key=lambda x: (ip_to_int(x.split("/")[0]), int(x.split("/")[1])),
    )
