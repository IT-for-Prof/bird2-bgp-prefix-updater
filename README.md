...
- **Защита от петли (own-infra)**: Собственные сети вычитаются из фида до записи (с hole-punch супернетов) и режутся в экспортных фильтрах BIRD; без инвентаря скрипт работает fail-closed
- **Кросс-источниковая дедупликация**: more-specific, накрытые супернетом того же класса community, выбрасываются (≈−30 % фида) без изменения покрытия; включено по умолчанию, с fail-closed проверкой фильтров пиров (см. [Дедупликация фида](#дедупликация-фида---aggregate-classes))
...
- `conf/own-infra.lst.example` — шаблон инвентаря собственных сетей, которые **никогда** не анонсируются (реальный файл `/etc/bird/own-infra.lst` не трекается; см. [Исключение собственной инфраструктуры](#исключение-собственной-инфраструктуры-anti-loop)).
...
```
Названия сервисов сравниваются без учёта регистра, но для читаемости лучше писать их так же, как в AWS `ip-ranges.json`.
Не очищайте фильтр без отдельной проверки: весь список AWS намного шире CloudFront и может сильно изменить маршрутизацию.

## Исключение собственной инфраструктуры (anti-loop)

Источник фида **никогда не должен анонсировать собственные сети**. Если в фид попадает префикс, покрывающий публичный IP вашего egress/next-hop, у потребителей возникает **маршрутная петля**: next-hop маршрута лежит внутри самого маршрута. Такие блоки могут легитимно присутствовать во внешних списках (например RU-список RIPEstat, RKN), поэтому чинить один источник бессмысленно — защита сделана **на уровне набора** (source-agnostic).
...
Список ведётся **вручную** в `/etc/bird/own-infra.lst` (путь переопределяется `OWN_INFRA_FILE`). Это ваш инвентарь публичных IP **egress/туннельных эндпоинтов** — добавляйте сеть узла при его вводе в строй. В git трекается только шаблон `conf/own-infra.lst.example`; реальный `/etc/bird/own-infra.lst` **не хранится в репозитории и не перезаписывается** при `git pull` (а сам путь `conf/own-infra.lst` ещё и в `.gitignore` — страховка от случайного коммита реальных данных).

Формат файла:
- один IPv4-адрес (станет `/32`) или CIDR на строку;
- `#` — комментарий, как с начала строки, так и **инлайн**: `198.51.100.0/24  # node A egress`;
- **только IPv4/CIDR, hostname не поддерживается** (DNS недетерминирован и сетезависим — провал/подмена резолва = own-сеть не вычтена = петля);
- узел с **множеством адресов** — задайте один покрывающий CIDR или каждый `/32` явно, не полагайтесь на DNS;
- нераспознанные строки пропускаются с WARNING (смотрите лог) и потому **не защищаются** — держите список чистым.

**Fail-closed:** встроенных дефолтов нет, поэтому файл обязателен. Если `/etc/bird/own-infra.lst` отсутствует или не содержит ни одного валидного префикса, скрипт **отказывается публиковать фид** (`exit 1`, старый файл сохраняется). Сам файл скрипт **не создаёт**.

Что вносить, а что нет:
- **Вносить:** next-hop/egress и публичные IP туннельных эндпоинтов; при необходимости — префикс underlay/транзита, через который downstream достигает next-hop.
- **Не вносить:** downstream BGP-роутеры — это потребители фида, защищённые собственными connected-маршрутами и входным фильтром. Исключение — если у такого роутера есть интерфейсы в адресном пространстве, которое может оказаться в источнике.

> Примечание: конвейер работает с IPv4 (фид v4-only). IPv6-эндпоинты вне области этого механизма.
...
## Дедупликация фида (`--aggregate-classes`)

`collapse` объединяет соседние/вложенные префиксы **только внутри одного источника**. Поэтому `/32` из одного списка (например заблокированный хост, community `200`) остаётся в фиде, даже если он целиком лежит внутри `/24` из другого источника (например CDN-блок `384` или РКН-подсеть `210`). На реальном фиде таких «накрытых» more-specific около **30%** (≈33 тыс. из 111 тыс.).

Дедупликация выбрасывает more-specific `P`, если его накрывает менее специфичный супернет `S` из того же **класса community** — диапазона, который каждый export-фильтр принимает целиком. Классы задаются флагом и по умолчанию равны диапазонам штатных фильтров:

```bash
# Включено по умолчанию — эквивалент:
python3 src/prefix_updater.py --aggregate-classes 100-199,200-399
# Полностью отключить (отдавать все more-specific):
python3 src/prefix_updater.py --no-aggregate
```

Почему это безопасно: фильтры различают community **диапазонами**, поэтому если `S` несёт все классы community, что и `P`, любой фильтр, принимающий `P`, примет и `S`. Longest-match на клиенте даёт тот же next-hop — покрытие не меняется, маршрутов меньше (на vps02: 111 048 → 77 337, −30 %).

**Защита (fail-closed).** Перед применением скрипт сканирует `--peers-dir` (по умолчанию `/etc/bird/peers.d`) и проверяет каждый export-фильтр пиров по реестру `FILTER_RANGES`:

- фильтр, чей accept-диапазон **частично пересекает** класс (например `export_blocked_only` 200–299 против класса 200–399), разрезал бы класс и привёл бы к потере маршрута — такой случай блокируется;
- неизвестный именованный фильтр и inline-фильтр со ссылкой на `bgp_community` тоже отклоняются (их диапазон недоказуем);
- inline-фильтр «отдать всё» (дефолт шаблона `t_client`) безопасен и игнорируется.

При **явном** `--aggregate-classes` несовместимость — фатальная ошибка (прогон прерывается). На **дефолтном** пути скрипт деградирует мягко: предупреждает и отдаёт полный недедуплицированный фид, чтобы изменение конфигурации постороннего пира не остановило обновления.

> ⚠️ Если добавляете пир с фильтром, который выделяет под-диапазон внутри класса (`export_blocked_only`, `export_services_only` или собственный по `bgp_community`), либо сузьте классы (`--aggregate-classes 100-199,200-299,300-399`), либо отключите дедуп (`--no-aggregate`) — иначе под-диапазонному пиру не хватит маршрутов, накрытых супернетом из соседнего под-диапазона.
...
- Для принудительного обновления кэша используйте флаг `--force-refresh`:
  ...
  ```
- Дедупликация фида включена по умолчанию; отключить — `--no-aggregate`, изменить классы — `--aggregate-classes` (см. [Дедупликация фида](#дедупликация-фида---aggregate-classes)).
