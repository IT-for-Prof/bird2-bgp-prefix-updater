...
## [Unreleased]
### Added
- **Optional feed dedup of covered more-specifics (`--aggregate-classes`).** Off by default. `collapse_networks` only dedups within a single source, so a `/32` from one list nested inside a `/24` from another survives — on the live feed ~33k such routes (30%) are blocked `/32`s sitting under a foreign-service or CDN `/24`. Passing `--aggregate-classes 100-199,200-399` drops a more-specific when a covering supernet shares its community *class* (a declared range that every export filter accepts/rejects as a whole), cutting the feed ~30% with identical longest-match forwarding for peers on `export_only_ru`/`export_blocked_lists`. **Fail-closed safety:** before applying, the run scans `--peers-dir` (default `/etc/bird/peers.d`) and aborts if any peer uses a filter whose accept-range only partially overlaps a class (e.g. `export_blocked_only` 200-299 vs class 200-399) or an unknown filter — those could still need a route dedup would drop. With no `--aggregate-classes`, only an exact community superset is dropped (universally safe, ~0 effect on real feeds), so default behavior is unchanged.
